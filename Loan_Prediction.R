...
in_train_rf <- createDataPartition(y = fraud_pp_w_result$loan_status, p = .75, 
                                list = FALSE) 
train_rf <- fraud_pp_w_result[in_train_rf, ] 
test_rf <- fraud_pp_w_result[-in_train_rf, ]
...
confusionMatrix(train_rf$loan_status, rf_train_pred, positive = "Yes")

# Predict on Test set
rf_test_pred <- predict(rf_model, test_rf)
confusionMatrix(test_rf$loan_status, rf_test_pred, positive = "Yes")

# Plot the ROC curve
rf_probs <- predict(rf_model, test_rf, type = "prob")

rf_ROC <- roc(response = test_rf$loan_status, 
              predictor = rf_probs$Yes, 
              levels = levels(test_rf$loan_status))
plot(rf_ROC)
auc(rf_ROC)